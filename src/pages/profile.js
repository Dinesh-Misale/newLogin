import React, { useState } from "react";

const Profile = () => {
    var [data, setData] = useState({Image : "data:image/png;base64,",
                                    name : "", date: "", religion: "", height: ""})
    const doing = (e) => {
        e.preventDefault();
        document.querySelector(".profile-form").classList.add("change-dispaly");
    }
    const changeData = (e)=> {
        const newData = {...data};
        newData[e.target.name] = e.target.value;
        setData(newData);
    }
    return(
        <div className="profile-div">
            <img src={data.Image} alt="this is your profile" />
            <section>
                <p>{data.name}</p>
                <p>{data.date}</p>
                <p>{data.religion}</p>
                <p>{data.height}</p>
            </section>
            <form className="profile-form" method="post" action="/profiledata">
                
                <h1>Please Enter Your Personal Information</h1>
                <label>Select your profile picture</label>
                <input type="file"  className="image-input" name="Image" onInput={changeData} placeholder="select your profile picture" />
                <input type="text" onInput={changeData} name="name" placeholder="Eneter your name" />
                <input type="date" onInput={changeData} name="date" />
                <input type="text" placeholder="Religion" onInput={changeData} name="religion"/>
                <input type="text" placeholder="Height" onInput={changeData} name="height" />
                <button type="submit" className="profile-button" onClick={doing}>Submit</button>
             </form>
        </div>
    );
}

export default Profile;